    ...
    <section className="overflow-hidden bg-[#f7f7f4]">
      <div className="mx-auto grid min-h-[650px] max-w-7xl items-center gap-14 px-6 py-16 lg:grid-cols-[1.05fr_.95fr] lg:px-8 lg:py-24">
        <div className="order-2 lg:order-1">
          <span className="inline-flex rounded-full border border-slate-200 bg-white px-4 py-2 text-xs font-bold text-slate-600 shadow-sm">انتخاب‌های تازه برای سبک زندگی امروز</span>
          <h1 className="mt-6 max-w-2xl text-5xl font-black leading-[1.15] tracking-tight text-slate-950 sm:text-6xl">
            چیزهای خوب،
            <span className="mt-2 block text-slate-500">ساده‌تر انتخاب می‌شوند.</span>
          </h1>
          ...
          <div className="mt-9 flex flex-col gap-3 sm:flex-row">
            <a href="#products" className="rounded-full bg-slate-950 px-7 py-3.5 text-center text-sm font-bold text-white transition hover:-translate-y-0.5 hover:bg-slate-800">مشاهده محصولات</a>
            <Link href="/login" className="rounded-full border border-slate-300 bg-white px-7 py-3.5 text-center text-sm font-bold text-slate-800 transition hover:border-slate-400">حساب کاربری من</Link>
          </div>
        ...
        <div className="order-1 lg:order-2">
          <div className="relative mx-auto aspect-square max-w-xl overflow-hidden rounded-[2rem] bg-slate-950 p-6 shadow-2xl shadow-slate-300/40">
            <div className="absolute -right-14 -top-14 h-48 w-48 rounded-full bg-emerald-300/30 blur-3xl" />
            <div className="absolute -bottom-16 -left-10 h-56 w-56 rounded-full bg-indigo-400/20 blur-3xl" />
